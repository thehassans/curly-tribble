...
async function markAgentClosingOrders({
  remitId,
  paidAt,
  deliveredOrderIds = [],
  cancelledOrderIds = [],
  ...
  if (!ids.length || !remitId || !paidAt) return;
  await Order.updateMany(
    { _id: { $in: ids } },
    {
      $set: {
        "agentClosing.paidAt": paidAt,
        "agentClosing.remit": remitId,
      },
    }
  );
}

async function markDriverClosingOrders({
  paymentSource,
  paymentRef,
  paidAt,
  deliveredOrderIds = [],
  ...
  const ids = toObjectIdList([...deliveredOrderIds, ...cancelledOrderIds]);
  if (!ids.length || !paymentRef || !paidAt || !paymentSource) return;
  await Order.updateMany(
    { _id: { $in: ids } },
    {
      $set: {
        "driverClosing.paidAt": paidAt,
        "driverClosing.paymentSource": paymentSource,
        "driverClosing.paymentRef": paymentRef,
      },
    }
  );
}
...
async function buildAgentClosingOrderData({ agentId, paidAt = new Date() }) {
  ...
  const deliveredOrders = await Order.find(
    ...
    .populate("items.productId", "name price baseCurrency")
    .lean();
  const summaryOrderMap = new Map();
  for (const order of [...submittedOrders, ...cancelledOrders, ...deliveredOrders]) {
    summaryOrderMap.set(String(order?._id || ""), order);
  }
  const summaryOrders = Array.from(summaryOrderMap.values());
  const totalSubmitted = summaryOrders.length;
  const totalCancelled = uniqueIdStrings(cancelledOrders.map((order) => order?._id)).length;
  const totalOrderValueAED = summaryOrders.reduce((sum, order) => {
...
async function buildDriverCommissionClosingData({
  ...
  const deliveredOrderIds = uniqueIdStrings(
    internalOrders.map((order) => order?._id)
  );
  const cancelledOrderIds = uniqueIdStrings(
    cancelledOrders.map((order) => order?._id)
  );
...
async function buildDriverCommissionClosingDataFromStoredOrders({
  driver,
  paidAt = new Date(),
  rangeStart = null,
  rangeEnd = null,
...
async function createDriverCommissionClosingPdf({
  ...
  const closing =
    closingData ||
    (await buildDriverCommissionClosingData({
      driver,
      ownerId,
      partnerId,
      paidAt,
  ...
  async (req, res) => {
    try {
      ...
      const me = await User.findById(req.user.id).select(
        "createdBy country assignedManager"
      );
      const ownerId = String(me?.createdBy || "");
      if (!ownerId)
        return res.status(400).json({ message: "No workspace owner" });
      ...
      let mgrDoc = null;
      const assignedManagerId = String(me?.assignedManager || "");
      if (managerId) {
        if (assignedManagerId && String(managerId) !== assignedManagerId) {
          return res
            .status(403)
            .json({ message: "Not allowed - manager mismatch" });
        }
        ...
        if (mgr && mgr.role === "manager" && String(mgr.createdBy) === ownerId) {
          managerRef = String(mgr._id);
          mgrDoc = mgr;
        }
      }
      ...
      if (req.user.role === "manager") {
        ...
        try {
          ...
          if (Number(r.driverCommission || 0) > 0) {
            const closingData =
              Array.isArray(r.closingOrderIds) && r.closingOrderIds.length
                ? await buildDriverCommissionClosingDataFromStoredOrders({
                    driver,
                    paidAt: r.paidAt || r.acceptedAt,
                    rangeStart: r.fromDate,
                    rangeEnd: r.toDate || r.paidAt || r.acceptedAt,
                    deliveredOrderIds: r.closingOrderIds,
                    cancelledOrderIds: r.closingCancelledOrderIds,
                  })
                : await buildDriverCommissionClosingData({
                    driver,
                    ownerId: r.owner,
            ...
            r.closingOrderIds = toObjectIdList(closing.deliveredOrderIds || []);
            r.closingCancelledOrderIds = toObjectIdList(
              closing.cancelledOrderIds || []
            );
            r.closingOrderCount = Number(closing.totalDelivered || closing.orderCount || 0);
            r.closingCancelledCount = Number(closing.totalCancelled || 0);
            await markDriverClosingOrders({
              paymentSource: "remittance",
              paymentRef: r._id,
              paidAt: r.paidAt || r.acceptedAt,
              deliveredOrderIds: closing.deliveredOrderIds,
              cancelledOrderIds: closing.cancelledOrderIds,
            });
            if (driver) {
              if (!driver.driverProfile) driver.driverProfile = {};
              driver.driverProfile.paidCommission =
                Number(driver.driverProfile.paidCommission || 0) +
                Number(r.driverCommission || r.amount || 0);
  ...
  async (req, res) => {
    try {
      ...
      const remit = new AgentRemit({
        ...
        sentAt: paidAt,
        sentBy: req.user.id,
        closingOrderIds: toObjectIdList(closingData.deliveredOrderIds || []),
      ...
      await remit.save();
      await markAgentClosingOrders({
        remitId: remit._id,
        paidAt,
        deliveredOrderIds: closingData.deliveredOrderIds,
        cancelledOrderIds: closingData.cancelledOrderIds,
      });
  ...
  async (req, res) => {
    try {
      ...
      const driver = await User.findOne({ _id: id, role: "driver" }).select(
        "firstName lastName phone createdBy assignedManager country driverProfile"
      );
      if (!driver) return res.status(404).json({ message: "Driver not found" });
      let ownerId = String(driver.createdBy || "");
      if (req.user.role === "user") {
      ...
      const preview = await buildDriverCommissionClosingData({
        driver,
        ownerId,
        paidAt: new Date(),
      });
      ...
      console.error("Driver commission preview error:", err);
      return res.status(500).json({ message: "Failed to load commission preview" });
    }
  }
);
    ...
    try {
      ...
      if (req.user.role === "manager") {
        const driverIds = await User.find(
          { role: "driver", createdBy: ownerId, assignedManager: req.user.id },
          { _id: 1 }
        )
          .lean()
          .then((xs) => xs.map((x) => x._id));
        if (!driverIds.length) return res.json({ requests: [] });
      ...
      const items = await DriverCommissionRequest.find(match)
        .sort({ createdAt: -1 })
        .limit(200)
        .populate("driver", "firstName lastName phone country assignedManager")
        .populate("approvedBy", "firstName lastName email")
  ...
  async (req, res) => {
    try {
      ...
      if (req.user.role !== "admin" && String(doc.owner) !== ownerId)
        return res.status(403).json({ message: "Not allowed" });
      if (req.user.role === "manager") {
        const driver = await User.findOne({ _id: doc.driver, role: "driver" })
          .select("assignedManager createdBy")
          .lean();
        if (
          !driver ||
          String(driver.createdBy || "") !== ownerId ||
          String(driver.assignedManager || "") !== String(req.user.id)
        ) {
          return res.status(403).json({ message: "Not allowed" });
        }
      }
    ...
    try {
      ...
      if (req.user.role === "manager") {
        ...
        const me = await User.findById(req.user.id)
          ...
          .lean();
        driverCond.createdBy = me?.createdBy || req.user.id;
        driverCond.assignedManager = req.user.id;
        const assigned =
          Array.isArray(me?.assignedCountries) && me.assignedCountries.length
            ? me.assignedCountries
            : me?.assignedCountry
          ...
          $addFields: {
                ...
                $filter: {
                  ...
                  cond: {
                    $and: [
                      ...
                      { $eq: [{ $ifNull: ["$$this.driverClosing.paidAt", null] }, null] },
                    ],
                  },
                },
              },
            ...
            deliveredOrders: {
              $filter: {
                ...
                cond: {
                  $and: [
                    { $eq: ["$$this.shipmentStatus", "delivered"] },
                    { $eq: [{ $ifNull: ["$$this.driverClosing.paidAt", null] }, null] },
                  ],
                },
              },
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const paidAt = new Date();
      const closingData = await buildDriverCommissionClosingData({
        driver,
        ownerId: ownerId || driver.createdBy,
        paidAt,
      });
      const amt = Number(closingData?.deliveredCommission || 0);
      if (!Number.isFinite(amt) || amt <= 0) {
        return res.status(400).json({
          message: "No delivered driver commission is available to pay",
        });
      }
      if (
        Number.isFinite(requestedAmount) &&
        ...
        return res.status(400).json({
          message:
            "Commission total changed. Please review the latest delivered orders before paying.",
        });
      }

      // Create a remittance record marking commission payment to driver
      const noteText = `Commission closing paid for ${closingData.totalDelivered} delivered orders out of ${closingData.totalSubmitted} total orders (${closingData.totalCancelled} cancelled). Total order amount ${closingData.currency} ${Number(
        closingData.totalOrderValue || 0
      ).toFixed(2)}, delivered order amount ${closingData.currency} ${Number(
        closingData.deliveredOrderValue || 0
      ).toFixed(2)}, paid commission ${closingData.currency} ${amt.toFixed(2)}`;
      ...
      const remit = new Remittance({
        ...
        amount: amt,
        driverCommission: amt,
        commissionRate: driver.driverProfile?.commissionPerOrder || 0,
        fromDate: closingData.rangeStart,
        toDate: closingData.rangeEnd,
        totalDeliveredOrders: Number(closingData.totalDelivered || 0),
        method: "transfer",
        note: noteText,
        status: status,
        paidToId: id,
        paidAt: status === "accepted" ? paidAt : null,
        closingOrderIds: toObjectIdList(closingData.deliveredOrderIds || []),
        closingCancelledOrderIds: toObjectIdList(
          closingData.cancelledOrderIds || []
        ),
...
router.get("/investors/earnings", auth, allowRoles("user", "admin"), async (req, res) => {
  try {
    ...
    const me = await User.findById(req.user.id).select("role createdBy").lean();
    const ownerId = String(
      me?.role === "user" ? me?._id : me?.createdBy || me?._id || ""
    );
    if (!ownerId) {
      return res.json({ investors: [], stats: { totalInvested: 0, totalReturns: 0, totalProfit: 0, pendingPayouts: 0, activeInvestors: 0 } });
    }

    // Build user query for investors (scoped to this workspace)
    const userQuery = { role: "investor", createdBy: new mongoose.Types.ObjectId(ownerId) };
    if (status === "active") userQuery["investorProfile.status"] = "active";
    if (status === "inactive") userQuery["investorProfile.status"] = "inactive";
    if (status === "completed") userQuery["investorProfile.status"] = "completed";
    ...
    const bonusAgg = await InvestorBonus.aggregate([
      ...
      { $group: { _id: "$investorId", total: { $sum: "$amount" } } },
    ]);
    const bonusMap = {};
    bonusAgg.forEach((b) => {
      bonusMap[String(b._id)] = Number(b.total || 0);
    });
    ...
    const profitAgg = await Order.aggregate([
      { $match: orderMatch },
      { $group: { _id: "$investorProfit.investor", total: { $sum: "$investorProfit.profitAmount" } } },
    ]);
    const profitMap = {};
    profitAgg.forEach((p) => {
      profitMap[String(p._id)] = Number(p.total || 0);
    });

    // Reserved payout totals per investor (approved + pending)
    const reservedAgg = await PayoutRequest.aggregate([
      {
        $match: {
          requesterType: "investor",
          requesterId: { $in: investorIds },
          status: { $in: ["approved", "pending"] },
        },
      },
      { $group: { _id: "$requesterId", total: { $sum: "$amount" } } },
    ]);
    const reservedMap = {};
    reservedAgg.forEach((p) => {
      reservedMap[String(p._id)] = Number(p.total || 0);
    });
    ...
    const earningsData = investors.map((inv) => {
      ...
      const orderProfit = profitMap[String(inv._id)] ?? profile.earnedProfit ?? 0;
      const bonusTotal = bonusMap[String(inv._id)] || 0;
      ...
      const reserved = reservedMap[String(inv._id)] || 0;
      const availableBalance = Math.max(0, Number(earnedProfit || 0) - Number(reserved || 0));
    ...
    // Get pending payouts (scoped to this workspace)
...
router.post("/investors/:id/bonus", auth, allowRoles("user", "admin"), async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const currency = String(investor?.investorProfile?.currency || "SAR");

    const doc = new InvestorBonus({
      ownerId,
      investorId: id,
      amount,
      currency,
    ...
    return res.status(201).json({ success: true, bonus: doc });
  } catch (error) {
    console.error("Send Investor Bonus Error:", error);
    return res.status(500).json({ message: "Failed to send bonus" });
  }
});
...
router.get("/investors/payout-requests", auth, allowRoles("user", "admin"), async (req, res) => {
  try {
    ...
    if (!ownerId) return res.json({ requests: [] });
    ...
    const enriched = requests.map((r) => ({
      ...r,
      investorName: investorNameMap[String(r.requesterId)] || "Unknown",
    }));
...
router.post("/investors/payout-requests", auth, allowRoles("investor"), async (req, res) => {
  try {
    ...
    // Check available balance: (real order profit + bonuses) - reserved payouts
    const user = await User.findById(req.user.id).select("firstName lastName investorProfile createdBy").lean();
    const ownerId = String(user?.createdBy || "");

    const profitAgg = await Order.aggregate([
      {
        $match: {
          "investorProfit.investor": new mongoose.Types.ObjectId(req.user.id),
          "investorProfit.isPending": false,
          "investorProfit.profitAmount": { $gt: 0 },
          $or: [{ shipmentStatus: "delivered" }, { status: "done" }],
        },
    ...
    const orderProfit = Number(profitAgg?.[0]?.total || 0);
        ...
        const bonusAgg = await InvestorBonus.aggregate([
          {
            $match: {
              ownerId: new mongoose.Types.ObjectId(ownerId),
              investorId: new mongoose.Types.ObjectId(req.user.id),
            },
          },
          { $group: { _id: null, total: { $sum: "$amount" } } },
        ]);
    ...
    const earnedProfit = orderProfit + bonusTotal;
    const reservedAgg = await PayoutRequest.aggregate([
      {
        $match: {
          requesterType: "investor",
