...
// Public webhook endpoints for WhatsApp Cloud API (Meta)
// NOTE: These endpoints must be publicly accessible for Meta to deliver webhooks.
router.get('/webhook', (req, res) => {
  try {
    ...
    if (!verifyToken) {
      console.warn('[wa webhook] WA_CLOUD_VERIFY_TOKEN is not set in environment')
      return res.status(403).send('Forbidden: verify token not configured')
    }
    if (mode === 'subscribe' && token === verifyToken) {
      console.log('[wa webhook] verified OK, returning challenge')
      return res.status(200).send(String(challenge || ''))
    }
    console.warn('[wa webhook] verification failed', { mode, tokenMatch: token === verifyToken })
    return res.status(403).send('Forbidden')
  } catch (err) {
    console.error('[wa webhook GET] error', err?.message || err)
...
router.post('/webhook', async (req, res) => {
  try {
    ...
    await waService.handleWebhook(req)
...
router.use((req, res, next) => {
  try{
    const p = req.path || ''
    if (p === '/mark-read' || p === '/status' || p.startsWith('/webhook')) return next();
  }catch{}
...
// Helper to lazy-load the official WhatsApp Cloud API service
const getWaService = async () => (await import('../services/whatsappCloud.js')).default;

router.get('/status', auth, async (_req, res) => {
  const waService = await getWaService();
  const st = await waService.getStatus();
  // Compatibility with existing UI: expose 'connected' and 'number'
  res.json({
...
router.get('/events', auth, async (req, res) => {
  return res.status(410).json({
    ok: false,
    error: 'deprecated',
    message: 'SSE events are not supported for WhatsApp Cloud API. Use webhooks + socket events instead.',
  })
});
...
router.post('/react', auth, async (req, res) => {
  try{
    ...
    try{
      await waService.sendReaction(jid, id, emoji || '❤')
      return res.json({ ok: true })
    }catch(err){
      const msg = String(err?.message || 'failed')
      if (msg.startsWith('wa-cloud-unsupported:')) {
...
router.post('/send-media', auth, multerArray('files', 30), async (req, res) => {
  ...
  try{
    ...
    const key = String(jid)
    const last = chains.get(key) || Promise.resolve()
    const p = last.then(() => waService.sendMedia(jid, req.files || [], { caption: caption || '' }))
    chains.set(key, p.finally(() => { if (chains.get(key) === p) chains.delete(key) }))
    const r = await p
    res.json(r);
  }catch(err){
