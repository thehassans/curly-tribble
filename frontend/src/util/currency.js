...
const CURRENCY_SYMBOLS = (() => {
  ...
  try {
    for (const c of COUNTRY_LIST || []) {
      ...
      const sym = String(c?.currencySymbol || '').trim()
      if (code && sym && !m[code]) m[code] = sym
    }
  } catch {}
  return m
})()
...
export function currencySymbol(code) {
  ...
  if (c === 'SAR') return resolveSarSymbol()
  return CURRENCY_SYMBOLS[c] || c
}

export function formatMoney(amount, code = 'SAR', opts = {}) {
  const c = String(code || 'SAR').toUpperCase()
  const symbol = currencySymbol(c)
  const value = Number(amount || 0)
  const min = Number.isFinite(opts.minimumFractionDigits) ? opts.minimumFractionDigits : 2
  const max = Number.isFinite(opts.maximumFractionDigits) ? opts.maximumFractionDigits : 2
