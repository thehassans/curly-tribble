  ...
  return (
    <div className="min-h-screen bg-[#F8F9FC] font-sans text-gray-900">
      ...
      <div className="lg:hidden">
          ...
          {(images.length + (hasVideo ? 1 : 0)) > 1 && (
            <div className="absolute bottom-4 left-0 right-0 z-20 flex justify-center items-center gap-1.5">
              {[...Array(images.length + (hasVideo ? 1 : 0))].map((_, i) => (
                <div key={i} className={`rounded-full transition-all duration-300 ${i === selectedImage ? 'w-5 h-2 bg-white shadow-md' : 'w-2 h-2 bg-white/50'}`} />
              ))}
            </div>
          )}
        ...
        <div className="bg-gradient-to-r from-emerald-500 to-green-500 px-4 py-2.5 flex items-center justify-center gap-2">
          <svg className="w-4 h-4 text-white flex-shrink-0" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" /></svg>
          <span className="text-white text-xs font-bold uppercase tracking-widest">Free Delivery</span>
          <span className="text-white/70 text-[10px] font-medium">on this order</span>
        </div>
        ...
        <div className="px-4 pt-4 pb-28">
          ...
          {hasVideo && (
            <div className="mb-4 -mx-4 px-4">
              <div className="relative bg-black overflow-hidden rounded-3xl shadow-xl">
                <video
                  src={videoUrl}
                  controls
                  loop
