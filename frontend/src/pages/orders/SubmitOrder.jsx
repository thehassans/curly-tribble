...
export default function SubmitOrder() {
  ...
  useEffect(() => {
    if (!Array.isArray(items) || items.length === 0) {
      setItems([{ productId: '', quantity: 1, searchText: '', showDropdown: false }])
    }
  }, [items])
  ...
  const itemsDetailed = useMemo(() => {
    try {
      return (items || []).map((it, idx) => {
        ...
        // Use salePrice if product is on sale and has a valid salePrice, otherwise use regular price
        const priceToUse = (p?.onSale && p?.salePrice && Number(p.salePrice) > 0) ? Number(p.salePrice) : Number(p?.price || 0)
        const unit = convertPrice(priceToUse, base, selectedCurrency)
        const qty = Math.max(1, Number(it?.quantity || 1))
        const amount = unit * qty
        return { idx, product: p, unit, qty, amount, isSalePrice: !!(p?.onSale && p?.salePrice && Number(p.salePrice) > 0) }
      })
    } catch {
      return []
    }
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      // If Stripe was selected and a payment link was returned, show it
      if (paymentMethod === 'stripe' && result?.stripePaymentUrl) {
        setStripePaymentLink(result.stripePaymentUrl)
        setShowPaymentLinkModal(true)
        setLinkCopied(false)
        setMsg('Order submitted — Stripe payment link generated!')
      } else {
  ...
  function currencyOf(o) {
    const ccyMap = { UAE:'AED', KSA:'SAR', Oman:'OMR', Bahrain:'BHD', Kuwait:'KWD', Qatar:'QAR', India:'INR', Pakistan:'PKR', Jordan:'JOD', USA:'USD', UK:'GBP', Canada:'CAD', Australia:'AUD' }
    if (o?.orderCountry && ccyMap[o.orderCountry]) return ccyMap[o.orderCountry]
  ...
  async function resolveFromCoords(lat, lng) {
    ...
    try {
      setResolving(true)
      // Use backend reverse geocode endpoint (uses LocationIQ → Google → OSM fallback)
      const result = await apiPost('/api/geocode/reverse', { lat: Number(lat), lng: Number(lng) })
      
      if (result.success) {
        const display = result.formatted_address || ''
        const cityGuess = result.city || ''
        const areaGuess = result.area || ''
        const countryName = result.country || ''
        
        // Country validation - check if resolved country matches selected order country
        if (form.orderCountry && countryName) {
          const countryMatches = {
            UAE: ['United Arab Emirates', 'UAE', 'الإمارات'],
            Oman: ['Oman', 'عمان'],
            KSA: ['Saudi Arabia', 'Kingdom of Saudi Arabia', 'KSA', 'السعودية'],
          ...
          const expectedNames = countryMatches[form.orderCountry] || [form.orderCountry]
          const isMatch = expectedNames.some(n => 
          ...
          if (!isMatch) {
            setLocationValidation({
              isValid: false,
              message: `Location is in ${countryName}, but order is for ${form.orderCountry}`,
            })
            return
          }
        }
        ...
        setLocationValidation({ 
          isValid: false, 
          message: result.error || 'Failed to resolve address' 
        })
        setResolveError(result.error || 'Failed to resolve address')
      }
    } catch (err) {
  ...
  return (
    <div>
          ...
          >
            ...
            >
              ...
              <div className="card" style={{ display: 'grid', gap: 10 }}>
                <div className="helper" style={{ opacity: 0.9 }}>Payment Method</div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button
                    ...
                    style={{
                      ...
                      border: paymentMethod === 'COD' ? '2px solid #00a884' : '1px solid var(--border)',
                      background: paymentMethod === 'COD' ? 'rgba(0,168,132,0.08)' : 'var(--panel)',
                  ...
                  <button
                    ...
                    style={{
                      flex: 1, padding: '10px 12px', borderRadius: 8, cursor: 'pointer',
                      border: paymentMethod === 'stripe' ? '2px solid #635bff' : '1px solid var(--border)',
                      background: paymentMethod === 'stripe' ? 'rgba(99,91,255,0.08)' : 'var(--panel)',
                ...
                {paymentMethod === 'stripe' && (
                  <div style={{ fontSize: 12, opacity: 0.7, padding: '4px 0' }}>
                    A payment link will be generated after order submission. You can copy and send it to the customer.
                  </div>
                )}
              </div>
              ...
              <button className="btn" type="submit" disabled={loading || !canCreateOrder}>
                {loading ? (
                  ...
                  paymentMethod === 'stripe' ? 'Submit Order & Generate Payment Link' : 'Submit Order'
                )}
              </button>
      ...
      {showPaymentLinkModal && stripePaymentLink && (
        <div
          style={{
        ...
        >
          <div
            ...
            style={{
              width: '100%', maxWidth: 480, display: 'grid', gap: 16,
              boxShadow: '0 20px 60px rgba(0,0,0,0.3)',
            }}
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ fontWeight: 700, fontSize: 16 }}>💳 Stripe Payment Link</div>
              <button
                type="button"
                onClick={() => setShowPaymentLinkModal(false)}
            ...
            <div style={{ fontSize: 13, opacity: 0.8 }}>
              Copy this link and send it to the customer to complete payment online.
            </div>
            <div style={{
              ...
              <button
                ...
                style={{
                  ...
                  background: linkCopied ? '#00a884' : undefined,
                }}
                onClick={() => {
                  try {
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              ...
              <button
                ...
                onClick={() => {
                  try {
                    const text = `Please complete your payment using this link:\n${stripePaymentLink}`
                          ...
                          {o.paymentMethod === 'stripe' && (
                            <>
                              <span style={{
                                ...
                                background: o.paymentStatus === 'paid' ? 'rgba(16,185,129,0.12)' : 'rgba(245,158,11,0.12)',
                                color: o.paymentStatus === 'paid' ? '#065f46' : '#b45309',
                              }}>
                                {o.paymentStatus === 'paid' ? '✓ Paid' : '⏳ Pending'}
                              </span>
                              {o.stripePaymentLink && o.paymentStatus !== 'paid' && (
                                <button
                                  className="btn secondary small"
                                  onClick={() => {
                                    try {
                                      navigator.clipboard.writeText(o.stripePaymentLink)
                                      setStripePaymentLink(o.stripePaymentLink)
                                      setShowPaymentLinkModal(true)
                                      setLinkCopied(false)
                                    } catch {}
                                  }}
