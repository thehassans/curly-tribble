...
import { getCachedCurrencyConfig, getCurrencyConfig, convert as fxConvert, currencySymbol, formatMoney } from '../../util/currency'
...
.product-card-taobao{background:#fff;border-radius:0;overflow:hidden;cursor:pointer;transition:all .2s ease;margin:2px}
.product-card-taobao:active{transform:scale(.98)}
.image-container{position:relative;aspect-ratio:1;background:#f5f5f5;overflow:hidden}
.wishlist-btn{position:absolute;top:8px;right:8px;width:34px;height:34px;border-radius:999px;border:1px solid rgba(0,0,0,.08);background:rgba(255,255,255,.92);backdrop-filter:blur(6px);display:grid;place-items:center;z-index:20;color:#ea580c;cursor:pointer;transition:transform .15s ease,background .15s ease}
.wishlist-btn:hover{transform:translateY(-1px);background:rgba(255,247,237,.98)}
.wishlist-btn svg{width:18px;height:18px}
.wishlist-btn.active{background:rgba(255,237,213,.98);border-color:rgba(234,88,12,.18)}
.image-container img{width:100%;height:100%;object-fit:cover;transition:transform .3s ease,opacity .3s ease}
.image-container img.hidden{opacity:0}
.video-player{position:absolute;top:0;left:0;width:100%;height:100%;object-fit:cover;opacity:0;transition:opacity .3s ease;z-index:1}
.video-player.visible{opacity:1}
.video-play-btn,.video-pause-btn{position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:40px;height:40px;background:rgba(0,0,0,.5);border:none;border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;z-index:10}
.video-play-btn svg,.video-pause-btn svg{width:18px;height:18px;margin-left:2px}
.discount-badge{position:absolute;top:0;left:0;background:linear-gradient(135deg,#ff4d4f 0%,#ff7875 100%);color:#fff;font-size:11px;font-weight:600;padding:2px 6px;border-radius:0 0 8px 0}
.video-indicator{position:absolute;bottom:6px;right:6px;width:22px;height:22px;background:rgba(0,0,0,.5);border-radius:4px;display:flex;align-items:center;justify-content:center}
.video-indicator svg{margin-left:1px}
.no-image-placeholder{width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:#f0f0f0}
.placeholder-icon{font-size:40px;opacity:.3}
.product-info{padding:8px 10px 10px}
.actions-row{margin-top:8px;display:flex;align-items:center;gap:8px}
.mini-add-btn{flex:1;height:32px;border-radius:8px;border:none;background:#ea580c;color:#fff;font-size:12px;font-weight:700;cursor:pointer}
.mini-add-btn.adding{background:#10b981}
.mini-add-btn:disabled{opacity:.8;cursor:not-allowed}
.mini-wa-btn{width:32px;height:32px;border-radius:8px;display:inline-flex;align-items:center;justify-content:center;background:#22c55e;color:#fff;text-decoration:none;flex:0 0 auto}
.mini-wa-btn svg{width:18px;height:18px}
.mini-wa-btn.disabled{background:#e5e7eb}
.product-name{font-size:13px;font-weight:400;color:#333;margin:0 0 4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;line-height:1.4}
.price-section{display:flex;flex-direction:column;gap:4px}
.price-row{display:flex;align-items:center;gap:2px;flex-wrap:wrap}
.currency-symbol{display:inline-flex;align-items:center;line-height:1;font-size:24px;font-weight:600;color:#ff4d4f}
.price-main{font-size:18px;font-weight:700;color:#ff4d4f;line-height:1}
.price-decimal{font-size:12px;font-weight:600;color:#ff4d4f}
.original-price{font-size:11px;color:#999;text-decoration:line-through;margin-left:6px}
.rotating-info-wrap{min-height:18px;display:flex;align-items:center}
.rotating-info-static{display:flex;align-items:center;gap:6px;min-width:0;flex-wrap:wrap}
.rotating-info-rating{display:inline-flex;align-items:center;gap:3px;min-width:0}
.rotating-info-meta{font-size:10px;color:#999}
.rotating-info-sales{font-size:11px;color:#999}
.rotating-info-delivery{display:inline-flex;align-items:center;gap:3px;font-size:11px;color:#16a34a;font-weight:600}
@media (max-width:480px){.product-card-taobao{margin:2px}.product-info{padding:6px 8px 8px}.product-name{font-size:12px}.price-main{font-size:16px}}
`

function ensureProductCardMiniStyles() {
  if (typeof document === 'undefined') return
  if (document.getElementById(PRODUCT_CARD_MINI_STYLE_ID)) return
  const style = document.createElement('style')
  style.id = PRODUCT_CARD_MINI_STYLE_ID
  style.textContent = PRODUCT_CARD_MINI_STYLES
  document.head.appendChild(style)
}

let productDetailPreloadPromise = null
function preloadProductDetail() {
  if (!productDetailPreloadPromise) {
    productDetailPreloadPromise = import('../../pages/ecommerce/ProductDetail.jsx').catch(() => null)
  }
  return productDetailPreloadPromise
}

let productCardMiniCurrencyPromise = null
function loadSharedCurrencyConfig() {
  if (!productCardMiniCurrencyPromise) {
    productCardMiniCurrencyPromise = getCurrencyConfig().catch(() => getCachedCurrencyConfig())
  }
  return productCardMiniCurrencyPromise
}
...
const RotatingInfo = memo(function RotatingInfo({ productId, salesCount }) {
  ...
  return (
    <div className="rotating-info-wrap">
      <div className="rotating-info-static">
        <span className="rotating-info-rating">
          {stars.map((s, i) => (
          ...
          <span className="rotating-info-meta">({reviewCount})</span>
        </span>
        <span className="rotating-info-sales">{salesCount} sold</span>
        <span className="rotating-info-delivery">
...
const ProductCardMini = memo(function ProductCardMini({ product, selectedCountry = 'SA', showVideo = false, showActions = false, currencyConfig = null }) {
  const navigate = useNavigate()
  const toast = useToast()
  const [ccyCfg, setCcyCfg] = useState(() => currencyConfig || getCachedCurrencyConfig())
  ...
  useEffect(() => {
    ensureProductCardMiniStyles()
  }, [])

  useEffect(() => {
    if (currencyConfig) {
      setCcyCfg(currencyConfig)
      return undefined
    }
    let alive = true
    loadSharedCurrencyConfig().then((cfg) => {
      if (alive && cfg) setCcyCfg(cfg)
    }).catch(() => {})
    return () => { alive = false }
  }, [currencyConfig])
