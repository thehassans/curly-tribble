  ...
  const renderEmptyState = () => (
    <div className="grid gap-5">
      ...
      <div>
        ...
        <div className={`grid ${isMobile ? 'grid-cols-2' : 'grid-cols-[220px_minmax(0,1fr)]'} gap-4`}>
          {!isMobile && (
          ...
          <div className={`grid ${isMobile ? 'grid-cols-2' : 'grid-cols-2 xl:grid-cols-3'} gap-3`}>
            {visibleCategoryCards.map((entry) => (
              <button
                key={entry.name}
                type="button"
                onClick={() => handleCategoryClick(entry.name)}
                className="rounded-[24px] border border-slate-200/80 bg-gradient-to-br from-white to-slate-50 shadow-[0_14px_40px_rgba(15,23,42,0.05)] p-3 flex items-center gap-3 text-left hover:translate-y-[-1px] transition-transform"
              >
      ...
      {open ? (
        <div className="fixed inset-0 z-[120] bg-white">
          <div className="px-3 pt-4 pb-3 border-b border-slate-100">
            <div className="flex items-center gap-2">
              <button type="button" onClick={() => setOpen(false)} className="w-10 h-10 rounded-full grid place-items-center text-slate-600">
              ...
              <form
                ...
                className="flex-1 flex items-center gap-2 rounded-full border-2 border-slate-900 px-3 py-2"
              >
                <input
                  ref={inputRef}
                  type="text"
